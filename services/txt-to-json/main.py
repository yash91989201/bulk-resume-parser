...
async def process_message(message, redis_client, keys):
    async with message.process():
        try:
            ...
            extracted_data = await extract_data(content, redis_client, keys)
            ...
            # Upload result
            minio_object_path = await upload_json_file(user_id, task_id, json_file_path=os.path.basename(json_file))
...
async def start_consumer(redis_client, keys):
        ...
        async with queue.iterator() as queue_iter:
            async for message in queue_iter:
                if shutdown_event.is_set():
                    break
                await process_message(message, redis_client, keys)
...
async def main():
    ...
    # Initialize Redis
    redis_client, keys = await initialize_redis()
    
    # Start consumer
    await start_consumer(redis_client, keys)
