...
async def get_parseable_files(task_id: str) -> List[ParseableFile]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status != 200:
                        logger.error(
                            f"Task {task_id}: API request failed with status {response.status}"
                        )
                        return []
                    ...
                    if response_data.get("status") != "SUCCESS":
                        logger.error(
                            f"Task {task_id}: API responded with error: {response_data}"
                        )
                        return []

                    parseable_files = []
                    for file_data in response_data.get("data", {}).get(
                        "parseableFiles", []
                    ):
                        try:
                            ...
                            logger.error(
                                f"Task {task_id}: Missing key in file data: {str(e)}"
                            )
                        except ValueError as e:
                            logger.error(
                                f"Task {task_id}: Invalid file status received - {str(e)}"
                            )
                    ...
                    if parseable_files:  # If we have parseable files, return them
                        ...
                        print(
                            f"Task {task_id}: No parseable files found, retrying... ({attempt + 1}/{retries})"
                        )

        except aiohttp.ClientError as e:
            logger.error(
                f"Task {task_id}: Connection error fetching parseable files: {str(e)}"
            )
        except json.JSONDecodeError:
            logger.error(f"Task {task_id}: Invalid JSON response from API")
        except Exception as e:
            logger.error(
                f"Task {task_id}: Unexpected error fetching parseable files: {str(e)}"
            )
    ...
    logger.error(
        f"Task {task_id}: Failed to fetch parseable files after {retries} retries"
    )
    return []
