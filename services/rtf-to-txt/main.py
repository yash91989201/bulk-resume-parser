...
async def process_message(message:AbstractIncomingMessage):
    ...
    try:
        ...
        # Step 1: Download RTF file from MinIO
        rtf_file_path = await download_rtf_file(file_path)

        # Step 2: Extract text from the RTF file
        txt_file_path = await extract_text_and_save(rtf_file_path, file_path)
        ...
        await cleanup_files([rtf_file_path, txt_file_path])
    ...
    while not shutdown_event.is_set():
        try:
            ...
            async with connection:
                ...
                # Declare the queue
                queue = await channel.declare_queue(QUEUES.RTF_TO_TXT, durable=True)
