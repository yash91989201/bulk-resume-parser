...
async def process_message(message:AbstractIncomingMessage):
    ...
    try:
        ...
        # Step 1: Download TXT file from MinIO (parseable-files bucket)
        txt_file_path = await download_txt_file(file_path)

        # Step 2: Upload txt file to MinIO (processed-txt-files bucket)
        minio_file_path = await upload_txt_file(txt_file_path, user_id, task_id)

        # Step 3: Send message to text-to-json queue
        ...
        await cleanup_files([txt_file_path])
    ...
    while not shutdown_event.is_set():
        try:
            ...
            async with connection:
                ...
                # Declare the queue
                queue = await channel.declare_queue(QUEUES.TXT_PASSTHROUGH, durable=True)
